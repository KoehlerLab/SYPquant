...
metadata <- metadata[metadata$USE & metadata$Hexanediol==4,]
...
for (file in metadata$File){
  ...
  poligon_per_file <- tmp_poligon[grepl(paste(metadata$Gonad[metadata$File==file],"_",sep=""),tmp_poligon$File) | grepl(paste(metadata$Gonad[metadata$File==file],"-",sep=""), tmp_poligon$File),]
  ...
  SC_per_file <- tmp_SC[grepl(paste(metadata$Gonad[metadata$File==file],"_",sep=""), tmp_SC$Tile) | grepl(paste(metadata$Gonad[metadata$File==file],"-",sep=""), tmp_SC$Tile),]
...
Vol2 <- ggplot(df[df$full_path %in% unique(df$full_path)[9:16] & df$VolumeNucleus.um3.<10,]) + geom_point(aes(x = Cx_rotated, y = Cy_rotated, color=VolumeNucleus.um3.)) + 
  facet_wrap(~ full_path, ncol=4) + scale_colour_viridis(option="D") + coord_fixed()
Vol2

Vol3 <- ggplot(df[df$full_path %in% unique(df$full_path)[9:16] & df$VolumeNucleus.um3.>10 & df$VolumeNucleus.um3.<60,]) + geom_point(aes(x = Cx_rotated, y = Cy_rotated, color=VolumeNucleus.um3.)) + 
  facet_wrap(~ full_path, ncol=4) + scale_colour_viridis(option="D") + coord_fixed()
Vol3
...
before <- ggplot(df[df$full_path %in% unique(df$full_path)[17:24],], aes(x = Cx_rotated, y = Cy_rotated)) + geom_point(alpha=0.3, color='darkred') +
  facet_wrap(~ full_path, ncol=4) + scale_colour_viridis(option="D") + coord_fixed()

after <- ggplot(df[df$full_path %in% unique(df$full_path)[17:24] & df$Sphericity>0.4,], aes(x = Cx_rotated, y = Cy_rotated)) + geom_point(alpha=0.3, color='darkred') + 
  facet_wrap(~ full_path, ncol=4) + scale_colour_viridis(option="D") + coord_fixed()
...
####################################
#Normalise pachytene length (remove TZ)

df$GonadLengthpxlNorm <- 0
for (file in unique(df$full_path)){
  df$GonadLengthpxlNorm[df$full_path==file]<-(df$GonadLengthpxl[df$full_path==file]-min(df$GonadLengthpxl[df$full_path==file & df$Cx_rotated>=0]))/(max(df$GonadLengthpxl[df$full_path==file])-min(df$GonadLengthpxl[df$full_path==file & df$Cx_rotated>=0])) #set 0 to begining of TZ
} #the min() does not make sense because it is a single value but I will keep it in order to make it understandable

NormalisedLength <- ggplot(data=df[df$GonadLengthpxlNorm>=0,]) + geom_point(aes(x=GonadLengthpxlNorm, y=MeanI_SC_SYP.Bkg), alpha=0.5) + geom_smooth(aes(x=GonadLengthpxlNorm, y=MeanI_SC_SYP.Bkg), method="loess") +
  facet_wrap(~ full_path, ncol=4) + xlab('Normalised Pachytene Length')
NormalisedLength

df <- df[df$GonadLengthpxlNorm>=0,]
#Normalise SC signal

df$MeanI_SC_SYP.Bkg.DAPInorm <- df$MeanI_SC_SYP.Bkg/df$I_Total_DAPI.Bkg  #per nucleus
...
#Binning 
##df <- df %>% group_by(full_path) %>% mutate(GonadLengthpxlNorm_bin= cut(GonadLengthpxlNorm, breaks=10))
...
ggplot(df.plot) + geom_jitter(aes(x=GonadLengthpxlNorm_bin, y=mean_ratioI_Total_SYPSC.Bkg.I_Total_SYP.Bkg , color=GT),height=0,width=.02, alpha=0.4, size=1) +
  ...
  labs(x="Normalised gonad length", y="Ratio of HA::SYP-1 sum intensity on SC vs Nucleus") + theme_cowplot() +
  theme(axis.text=element_text(size=10),axis.title=element_text(size=12)) 
...
ggplot(df.plot) + geom_jitter(aes(x=GonadLengthpxlNorm_bin, y=mean_ratioI_Total_SYPSC.Bkg.I_Total_SYPNuc.Bkg , color=GT),height=0,width=.02, alpha=0.4, size=1) +
  ...
  labs(x="Normalised gonad length", y="Ratio of HA::SYP-1 sum intensity on SC vs Nucleoplasm") + theme_cowplot() +
  theme(axis.text=element_text(size=10),axis.title=element_text(size=12)) 



final <- ggplot(df.plot) + geom_jitter(aes(x=GonadLengthpxlNorm_bin, y=mean_ratioI_Total_SYPSC.Bkg.I_Total_SYP.Bkg , color=GT),height=0,width=.02, alpha=0.4, size=1) +
  stat_summary(aes(x=GonadLengthpxlNorm_bin, y=mean_ratioI_Total_SYPSC.Bkg.I_Total_SYP.Bkg , color=GT),geom='line', linewidth=0.5, alpha=0.8) +
  stat_summary(aes(x=GonadLengthpxlNorm_bin, y=mean_ratioI_Total_SYPSC.Bkg.I_Total_SYP.Bkg , color=GT),geom='errorbar', width=0.05) +
  labs(x="Normalised pachytene length", y="Ratio of SYP-4::HA sum intensity on SC vs Nucleus") + theme_cowplot() +
  theme(axis.text=element_text(size=10),axis.title=element_text(size=12)) +  ylim(0.3,0.7) +
  scale_color_manual(values=c(WT_Color, D114_Color, AN90_Color)) +
  force_panelsizes(rows = unit(1.6, "in"), cols = unit(1.6, "in"))


pdf(file=paste(Sys.Date(),"_D114_9FA_SChexanediolSensitivity_MEANSD_errorbar_v4.pdf",sep = ""),useDingbats = TRUE) #in inches
final
dev.off()


final <- ggplot(df.plot[df.plot$GT%in%c('ie29-10', 'D114'),]) + geom_jitter(aes(x=GonadLengthpxlNorm_bin, y=mean_ratioI_Total_SYPSC.Bkg.I_Total_SYP.Bkg , color=GT),height=0,width=.02, alpha=0.4, size=1) +
  stat_summary(aes(x=GonadLengthpxlNorm_bin, y=mean_ratioI_Total_SYPSC.Bkg.I_Total_SYP.Bkg , color=GT),geom='line', linewidth=0.5, alpha=0.8) +
  stat_summary(aes(x=GonadLengthpxlNorm_bin, y=mean_ratioI_Total_SYPSC.Bkg.I_Total_SYP.Bkg , color=GT),geom='errorbar', width=0.05) +
  labs(x="Normalised pachytene length", y="Ratio of SYP-4::HA sum intensity on SC vs Nucleus") + theme_cowplot() +
  theme(axis.text=element_text(size=10),axis.title=element_text(size=12)) +  ylim(0.3,0.7) +
...
pdf(file=paste(Sys.Date(),"_D114_SChexanediolSensitivity_MEANSD_errorbar_v4.pdf",sep = ""),useDingbats = TRUE) #in inches
final
dev.off()


final <- ggplot(df.plot[df.plot$GT%in%c('ie29-10', 'AN90'),]) + geom_jitter(aes(x=GonadLengthpxlNorm_bin, y=mean_ratioI_Total_SYPSC.Bkg.I_Total_SYP.Bkg , color=GT),height=0,width=.02, alpha=0.4, size=1) +
  stat_summary(aes(x=GonadLengthpxlNorm_bin, y=mean_ratioI_Total_SYPSC.Bkg.I_Total_SYP.Bkg , color=GT),geom='line', linewidth=0.5, alpha=0.8) +
  stat_summary(aes(x=GonadLengthpxlNorm_bin, y=mean_ratioI_Total_SYPSC.Bkg.I_Total_SYP.Bkg , color=GT),geom='errorbar', width=0.05) +
  labs(x="Normalised pachytene length", y="Ratio of SYP-4::HA sum intensity on SC vs Nucleus") + theme_cowplot() +
  theme(axis.text=element_text(size=10),axis.title=element_text(size=12)) + ylim(0.3,0.7) +
...
pdf(file=paste(Sys.Date(),"_9FA_SChexanediolSensitivity_MEANSD_errorbar_v4.pdf",sep = ""),useDingbats = TRUE) #in inches
final
dev.off()